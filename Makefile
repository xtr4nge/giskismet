# This Makefile is for the giskismet extension to perl.
#
...
#     AUTHOR => [q[Joshua D. Abraham]]
#     BUILD_REQUIRES => {  }
#     EXE_FILES => [q[giskismet]]
#     INSTALLDIRS => q[site]
#     MAN1PODS => { giskismet.man.1=>q[blib/man1/giskismet.1p] }
#     NAME => q[giskismet]
#     PREREQ_PM => { DBD::SQLite=>q[1.11], XML::LibXML=>q[1.01], DBI=>q[1.50] }
#     VERSION => q[0.02]
#     clean => { FILES=>q[] }
...
pure_all :: $(INST_SCRIPT)/giskismet
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/giskismet 

$(INST_SCRIPT)/giskismet : giskismet $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/giskismet
	$(CP) giskismet $(INST_SCRIPT)/giskismet
	$(FIXIN) $(INST_SCRIPT)/giskismet
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/giskismet
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               giskismet' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.02' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           ~' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Joshua D. Abraham' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    DBD::SQLite:  1.11' >> META_new.yml
	$(NOECHO) $(ECHO) '    DBI:          1.50' >> META_new.yml
	$(NOECHO) $(ECHO) '    XML::LibXML:  1.01' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Joshua D. Abraham</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DBD::SQLite" VERSION="1.11" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DBI::" VERSION="1.5" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="XML::LibXML" VERSION="1.01" />' >> $(DISTNAME).ppd
